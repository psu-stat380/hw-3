...
The LASSO plot suggests a lambda of approximately -3.5 being optimal for the minimization of mean-squared error. According to the plot, this model has 4 variables. The ridge plot, on the other hand, uses a lambda of approximately -2 to minimize mean-squared error and includes all variables because ridge regression does not do variable selection.
...
LASSO regression best for data frames with a very large amount of columns. LASS regression uses mean-squared error to do variable selection.

Ridge regression does not do variable selection.

Stepwise selection uses 'AIC' to do variable selection.
